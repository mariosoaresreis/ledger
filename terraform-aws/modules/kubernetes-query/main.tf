...
resource "kubernetes_namespace" "ledger_query" {
  metadata {
    name   = "ledger-query"
    labels = { managed-by = "terraform", component = "query" }
  }
}
...
resource "kubernetes_config_map" "query_config" {
  ...
  data = {
    ...
    QUERY_DB_PORT                  = var.query_db_port
    ...
    template {
      ...
      spec {
        container {
          ...
          env_from { config_map_ref { name = kubernetes_config_map.query_config.metadata[0].name } }
          env_from { secret_ref    { name = kubernetes_secret.query_secret.metadata[0].name } }

          env { name = "JAVA_TOOL_OPTIONS"; value = "-Xmx320m -Xms128m -XX:+UseContainerSupport" }
